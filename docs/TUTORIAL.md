...
| [3. Your first guardrail](#part-3--your-first-guardrail) | `init`, `.cdec/`, `check`, `baseline` | Yes |
| [4. The rule catalogue](#part-4--the-rule-catalogue) | Dependency, layer, and shape rules | Yes |
| [5. Tagging code with constraints](#part-5--tagging-code-with-constraints) | Rule tags + `enforce` (Engine B) | Tags |
| [6. Freezing implementations](#part-6--freezing-implementations) | `lock` (Engine C) | Tags |
| [7. The reference gate](#part-7--the-reference-gate) | `reference test` — the strict backstop | Yes |
...
```
.cdec/
  config.yaml      project settings (language, source path, lock behaviour)
  rules.yaml       the rules you opt into  — starts empty
  baseline.yaml    accepted violations, each with its reason — starts empty
...
This records current violations into `.cdec/baseline.yaml`. They are reported as
...
Slower than `--update-baseline`, and much better on a codebase you intend to keep: each
...
Baseline resolution order: `--base-ref` → `--reference` → `config.yaml`
`baseline.reference` → `.cdec/reference.xmi`. With no baseline at all, `scope: diff` rules
are **skipped** rather than silently passing — the report lists them under "skipped".
...
```bash
cdec check --update-reference     # rewrites .cdec/reference.xmi
...
this one violation is fine, the rest still block. That is `cdec baseline`.
...
`V-` is a `cdec check` issue, `F-` a `cdec enforce` finding, `L-` a `cdec lock` violation.
The key hashes *what* the issue is — engine, rule, element, discriminator — and never where
it sits, so:

- re-running over unchanged code always gives the same key, and
- reformatting the file or moving the class does not invalidate a waiver you granted.
...
```bash
cdec baseline review --out review.txt
```
...
```bash
cdec baseline list                 # what's allowed, why, when, and by whom
cdec baseline remove V-DD3EA5B2    # withdraw — the issue blocks again
cdec baseline prune                # drop waivers whose issue no longer occurs
```
...
- **Lock violations (`L-`) cannot be waived here.** `cdec baseline allow` on one prints the
  privileged command instead (`cdec lock set --target … --force`). See
...
`cdec baseline allow <key> --reason "…"` without parsing prose.
...
`baseline.yaml`), `type`, `severity` (`error` | `warning` | `off`), `scope`
...
*implementation* honours it. This is **Engine B** (`cdec enforce`).

The rules in Part 4 look only at the model — names, types, references. Engine B goes
further: it re-parses the source and **inspects method bodies**.
...
cdec enforce: 2 conformance violation(s):
  - [factory] orders/billing.py:97: 'orders.CheckoutService.quick_receipt' constructs
    'Receipt' outside its designated factory (ReceiptFactory).
  - [no-instantiation] orders/billing.py:97: 'orders.CheckoutService.quick_receipt' is
    tagged @no_instantiation but constructs 'Receipt'.
```

Two rules, two independent findings, exit code `1`. The bundled demos ship this exact
seeded violation so you can see a real failure — route the call through the factory and the
run goes green.
...
### 5.5 Running Engines A and B together

```bash
cdec check --enforce
```

One invocation, one exit code. The engines share the rule *catalogue* and nothing else —
`enforce` never consults the reference model or a diff, so its findings are always about the
code as it stands right now. It does honour findings you accepted through
[`cdec baseline`](#37-accepting-a-violation--the-review-loop); pass `--no-baseline` to see
them all.
...
**Goal:** declare that a specific function body is settled and must not change. This is
**Engine C** (`cdec lock`). Python, C#, Odin, Lua and Julia — every language with an AST
fingerprinter. TypeScript and Svelte are refused by name.

Engines A and B ask "did the design drift?" and "does the code obey its tags?". Engine C
asks something narrower and stricter: **did this body change at all?**
...
This writes the approved digest into `.cdec/locks.yaml`:
...
Commit that file. Verify at any time:

```bash
cdec lock list          # what is frozen, and does it still match?
cdec lock list --all    # ALSO show everything that COULD be frozen
...
### 6.3 What `lock check` catches

| Kind | Headline | Meaning |
|---|---|---|
| `changed` | frozen implementation changed | The body's digest no longer matches. |
| `missing` | declared `@locked` but not baselined | Tagged but never recorded via `lock set`. |
...
`cdec lock set` is deliberately split into a safe operation and a privileged one:

```bash
cdec lock set                        # SAFE: only ADDS locks for newly tagged code.
                                     # Anyone can run it. It can never erase evidence
                                     # that frozen code changed.

cdec lock set --target orders.Receipt.formatted --force \
              --reason "approved settlement fix" --owner alice
                                     # PRIVILEGED: accepts a change to frozen code.
```

Only `--force` re-baselines a drifted digest or prunes a stale entry — and it produces a
**reviewable diff on `.cdec/locks.yaml`**. Put that file behind a CODEOWNERS entry and
...
```bash
cdec lock check --bypass --bypass-reason "hotfix INC-4412, follow-up ticket ARCH-88"
```
...
To freeze code you would rather not annotate (or cannot), use globs in `.cdec/config.yaml`:
...
### 6.8 It runs automatically

Once `.cdec/locks.yaml` has entries (or `lock.targets` is set), **`cdec check` runs Engine C
for you**. You do not need a separate CI step:

```bash
cdec check                # includes lock verification
cdec check --no-locks     # opt out
```
...
### 7.1 What it catches that `check` does not

This is the subtle and important part. The two mechanisms sit on **different comparison
engines**:

- `cdec check` reads the diff engine, which matches members by **signature** and only marks
  a matched member changed when its *rule tags* differ. It is therefore **blind** to
  access-level changes (`public` → `private`), modifier changes (`static`, `abstract`,
  `readonly`), and class-kind changes.
- `cdec reference test` runs a dedicated comparator that walks both models field by field
  and **does** catch all of those.

So a PR that flips a public method to private, or makes a concrete class abstract, **passes
`cdec check`** — even with `frozen-members` enabled — and **fails `cdec reference test`**.
That gap is the entire reason the reference gate exists as a separate mechanism.
...
```bash
cdec reference update             # re-snapshot from the current source
cdec reference test --json --output reference-report.json
cdec reference show               # open the viewer on code-vs-reference
```
...
**green additions** ("still to build"). That is deliberately the inverse of
...
| "Tagged code must actually honour its constraint" | `cdec enforce` |
| "*This function* is settled — nobody touches it" | `cdec lock` |

They compose. Many teams run the reference gate as a hard backstop *and* `check`/`enforce`
for the rules that need semantic meaning.
...
author a model  →  cdec propose  →  look & discuss  →  edit  →  propose again
                                                    ...
                                                    cdec reference set
                                                              ↓
                                            cdec check / reference test now
                                            constrain development toward it
```
...
From here on, `cdec check` and `cdec reference test` measure the code against the agreed
target. Because the target contains classes that do not exist yet, `cdec reference show`
...
**Goal:** make the guardrails automatic. Every gate communicates through **exit codes**, so
this works in any CI system without parsing output.

### 9.1 Generated helper scripts

`cdec` can emit ready-made wrappers that run `check` then `enforce` and propagate non-zero
exits — `cdec-ci.sh` and `cdec-ci.bat`. Generate them from the interactive session
(`cdec` with no subcommand → "Generate CI/CD scripts").
...
jobs:
  constraints:
    ...
    steps:
      ...
      # Engines A + B + C in one invocation.
      - name: Architectural constraints
        run: cdec check --base-ref origin/${{ github.base_ref }} --enforce --json-out lint.json

      # Hard structural backstop.
      - name: Reference gate
        run: cdec reference test
...
architecture:
  ...
  script:
    ...
    - cdec check --base-ref origin/$CI_MERGE_REQUEST_TARGET_BRANCH_NAME --enforce
    - cdec reference test
  artifacts:
    when: always
    paths: [lint.json]
...
2. **Week 2 — ratchet.** Add rules, run `cdec check --update-baseline`, commit the
   baseline. Existing problems are grandfathered; new ones fail.
3. **Week 3 — tag the crown jewels.** Add `@sealed` / `@immutable` / `@factory` to the
   classes where the design genuinely matters, turn on `--enforce`.
4. **Week 4 — lock the untouchables.** `@locked` on the handful of bodies that must not
   change; put `.cdec/locks.yaml` behind CODEOWNERS.
5. **Later — consider the reference gate**, once the architecture is stable enough that
...
|---|---|---|
| Target architecture | Building the wrong thing | `reference test` |
| Dependency & layering rules | Wiring it up wrongly | `check` |
| Rule tags | Ignoring a design constraint | `enforce` |
| Locks | Touching settled code | `lock check` |
...
Point the agent at `cdec check --enforce` as part of its build loop and it will self-correct
against your architecture instead of yours-in-theory.
...
Support is **layered**, and the layers are independent. A language always has a parser; tags,
`enforce` and `lock` each need extra machinery on top.

| Capability | Python | C# | Odin | Lua | Julia | TypeScript | Svelte |
|---|---|---|---|---|---|---|---|
| Parse to model | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| Diagrams, diff, `check` (Engine A) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| Rule tags | ✅ decorators | ✅ attributes | ✅ `//@cdec` | ✅ `---@cdec` | ✅ macros | ❌ | ❌ |
...
TypeScript and Svelte are fully supported for **modelling and Engine A** — parsing,
diagrams, diffing, dependency and layering rules. They have no rule tags yet, so `cdec
enforce` runs but has nothing to check, and `cdec lock` refuses by name.
...
exists (`cdec check --update-reference`). Skipped rules are listed in the report — they do
not fail silently.

**`cdec lock check` says `not-baselined`**
Something is tagged `@locked` but was never recorded. Run `cdec lock set`.

**`cdec lock check` says `algo-mismatch`**
The ledger was written by a different fingerprinter version. Re-baseline with
`cdec lock set --force` after reviewing.
...
true` is set in `config.yaml`, docstring edits count too.

**`cdec enforce` false positives in Python**
...
`examples/csharp_demo` each carry a fully-commented `rules.yaml`, tagged classes, a
committed lock, and one intentional seeded violation — so `cdec check` passes while
`cdec enforce` fails, exactly as the tooling intends.

```bash
make demo      # runs all three engines against both demos
```
