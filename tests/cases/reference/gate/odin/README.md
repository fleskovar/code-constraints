# The reference gate — the whole public shape is frozen (Odin)

## What this proves

Odin has no constraint tags in this case and needs none: the reference gate runs on the parsed model alone, so it protects an Odin codebase's public shape — struct fields and procedure signatures — with no annotation at all.
...
| `inputs/src/catalog/book.odin` | three deviations plus a new struct |
| `inputs/baseline/catalog/book.odin` | the reference shape |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `catalog.Book.rating` | `int` → `f64`: a field type change |
| `catalog.Book.isbn` | a new field |
| `catalog.Book.pages` | the proc gained a parameter |
| `catalog.Series` | a new struct |
...
| --- | --- | --- |
| `rating: int` → `rating: f64` | same name, different type | `attribute-changed` |
...
`pages :: proc(b: ^Book, hardback: bool)` belongs to `catalog.Book` because an Odin operation's owner is **the type of its first parameter**. The receiver is stripped from the signature, so the deviation reads `pages(hardback:bool):int`. Adding a *first* parameter of a different type would therefore move the procedure to another class entirely, and present as `operation-removed` + `operation-added` on two different elements.
...
```bash
make test-case CASE=reference/gate/odin
make debug-case CASE=reference/gate/odin
```
