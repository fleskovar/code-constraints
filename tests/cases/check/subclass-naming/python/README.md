# `subclass-naming` — concrete notifiers must end in `Notifier` (Python)
...
| `inputs/src/notifications/base.py` | the contract base |
| `inputs/src/notifications/email.py` | a conforming implementation |
| `inputs/src/notifications/legacy.py` | an ignored implementation |
| `inputs/src/notifications/slack.py` | the violation |
| `inputs/src/orders/order.py` | a class with no bases |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `notifications.Notification` | the base itself. It does not inherit from `Notification`, so clause 3 keeps it out |
| `notifications.EmailNotifier` | inherits the base and matches `.*Notifier$` — the passing row |
| `notifications.SlackHook` | inherits the base, fails the pattern. **The violation.** |
| `notifications.LegacyPager` | also fails the pattern, but is named in `ignore:` |
| `orders.Order` | no bases at all — never examined, whatever it is called |
...
| --- | --- | --- | --- |
| `notifications.Notification` | `[]` | ❌ no bases | not examined (clause 3) |
| `notifications.EmailNotifier` | `['Notification']` | ✅ | `fullmatch` succeeds → silent |
| `notifications.SlackHook` | `['Notification']` | ✅ | `fullmatch` fails → **Fires** |
| `notifications.LegacyPager` | `['Notification']` | ✅ | `fullmatch` fails, but `ignore:` exempts it |
| `orders.Order` | `[]` | ❌ no bases | not examined (clause 3) |
...
| --- | --- |
| `notifications.Notification` | not examined (clause 3) |
| `notifications.EmailNotifier` | `fullmatch` succeeds → silent |
| `notifications.LegacyPager` | `fullmatch` fails, but `ignore:` exempts it |
| `orders.Order` | not examined (clause 3) |
...
```bash
make test-case CASE=check/subclass-naming/python
make debug-case CASE=check/subclass-naming/python
```
