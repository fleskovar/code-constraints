# `subclass-naming` — concrete notifiers must end in `Notifier` (Svelte 5)
...
| `inputs/src/notifications/base.ts` | the abstract base, in a plain `.ts` module |
| `inputs/src/notifications/email.ts` | a conforming implementation |
| `inputs/src/notifications/legacy.ts` | an ignored implementation |
| `inputs/src/notifications/slack.ts` | the violation |
| `inputs/src/routes/Page.svelte` | a component — a class with no bases |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `notifications.Notification` | the abstract base; extends nothing |
| `notifications.EmailNotifier` | `extends Notification` and matches the pattern |
| `notifications.SlackHook` | `extends Notification`, fails the pattern. **The violation.** |
| `notifications.LegacyPager` | also fails, but is named in `ignore:` |
| `routes.Page` | a **component**. It is a class in the model, but it extends nothing, so it is never examined |
...
The class hierarchy lives in plain `.ts` modules, which is where Svelte 5 logic belongs anyway — the Svelte parser reads them with the same TypeScript grammar. The `.svelte` component is included to show that components join the same class namespace without disturbing the rule: a component has no base list, so it never matches.
...
```bash
make test-case CASE=check/subclass-naming/svelte
make debug-case CASE=check/subclass-naming/svelte
```
