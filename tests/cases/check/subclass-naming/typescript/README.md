# `subclass-naming` — concrete notifiers must end in `Notifier` (TypeScript)
...
| `inputs/src/notifications/base.ts` | the abstract base |
...
| `inputs/src/orders/order.ts` | a class with no `extends` |
...
| `orders.Order` | no `extends` — never examined |
...
```bash
make test-case CASE=check/subclass-naming/typescript
make debug-case CASE=check/subclass-naming/typescript
```
