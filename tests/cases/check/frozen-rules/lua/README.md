# `frozen-rules` — deleting the tag is the violation (Lua)
...
| `inputs/src/orders/billing.lua` | the drifted tags |
| `inputs/src/support/audit.lua` | an identical deletion outside `classes:` |
| `inputs/baseline/orders/billing.lua` | the agreed baseline — parsed as the OLD side of the diff |
| `inputs/baseline/support/audit.lua` | the agreed baseline — parsed as the OLD side of the diff |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `orders.Receipt` | `---@cdec sealed` deleted; `---@cdec immutable` kept |
| `orders.Receipt.subtotal` | `---@cdec no_side_effects` deleted from the method |
| `support.AuditEntry` | `---@cdec sealed` deleted identically — but outside `classes:` |
...
[
  ...
  {
    "element": "orders.Receipt",
    "member": "operation:subtotal(n:):|@no-side-effects",
    "rule": "freeze-architectural-tags",
    "severity": "error"
  }
]
...
**catalog id** (clause 6), not the language's spelling — `---@cdec sealed` becomes `sealed` and `---@cdec no_side_effects` becomes `no-side-effects`.
...
The operation signature is `subtotal(n:):` — empty on both sides of the colon, because **Lua has no type annotations**. It is still a perfectly good identity: the parameter *names* and their order are what distinguish one operation from another here, so renaming a parameter is a signature change in Lua exactly as it is elsewhere.
...
```bash
make test-case CASE=check/frozen-rules/lua
make debug-case CASE=check/frozen-rules/lua
```
