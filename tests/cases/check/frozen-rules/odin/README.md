# `frozen-rules` — deleting the tag is the violation (Odin)
...
| `inputs/src/orders/billing.odin` | the drifted tags |
| `inputs/src/support/audit.odin` | an identical deletion outside `classes:` |
| `inputs/baseline/orders/billing.odin` | the agreed baseline — parsed as the OLD side of the diff |
| `inputs/baseline/support/audit.odin` | the agreed baseline — parsed as the OLD side of the diff |
...
| `orders.Receipt.subtotal` | `//@cdec no_side_effects` deleted from the proc |
...
[
  ...
  {
    "element": "orders.Receipt",
    "member": "operation:subtotal(n:int):f64|@no-side-effects",
    "rule": "freeze-architectural-tags",
    "severity": "error"
  }
]
...
The tagged proc is `subtotal :: proc(r: ^Receipt, n: int)`. Odin declares procedures at package scope, so **an operation's owner is the type of its first parameter** — the tag lands on `orders.Receipt.subtotal`, which is where a reader expects to find it. The receiver is stripped from the signature, hence `subtotal(n:int):f64`.
...
```bash
make test-case CASE=check/frozen-rules/odin
make debug-case CASE=check/frozen-rules/odin
```
