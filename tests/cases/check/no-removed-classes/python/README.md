# `no-removed-classes` — a rename is a removal (Python)
...
```bash
make test-case CASE=check/no-removed-classes/python
make debug-case CASE=check/no-removed-classes/python
```
