# `forbidden-references` — the catalogue must not know the buyer (C#)
...
| `inputs/src/Catalog/Author.cs` | source under test |
...
| --- | --- |
| `Catalog.Book` | a `Customer` constructor parameter and property → `Users.Customer`. **The violation.** |
...
| --- | --- | --- |
| `Catalog.Book` | a `Customer` constructor parameter and property | `Users.Customer` |
...
```bash
make test-case CASE=check/forbidden-references/csharp
make debug-case CASE=check/forbidden-references/csharp
```
