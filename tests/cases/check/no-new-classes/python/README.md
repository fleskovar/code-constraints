# `no-new-classes` — a new type needs design review first (Python)
...
| `inputs/src/notifications/express.py` | source under test |
| `inputs/src/testing/fake.py` | source under test |
...
```bash
make test-case CASE=check/no-new-classes/python
make debug-case CASE=check/no-new-classes/python
```
