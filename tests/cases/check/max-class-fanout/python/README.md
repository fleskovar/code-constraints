# `max-class-fanout` — the hub class (Python)
...
| `inputs/src/app/root.py` | also over the limit, but ignored |
| `inputs/src/orders/checkout.py` | the hub — eight distinct references |
| `inputs/src/orders/collaborators.py` | eight leaf classes with no outgoing references |
| `inputs/src/orders/pricing.py` | four distinct references, one named twice |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `orders.CheckoutService` | eight distinct constructor parameter types. **The violation.** |
| `orders.PriceCalculator` | four distinct types, with `Cart` in two parameters — proves the count is of *distinct* targets |
...
```bash
make test-case CASE=check/max-class-fanout/python
make debug-case CASE=check/max-class-fanout/python
```
