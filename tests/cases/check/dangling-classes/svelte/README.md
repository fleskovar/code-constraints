# `dangling-classes` — dead code versus entry points (Svelte 5)
...
| `inputs/src/lib/cart.ts` | a class with an incoming reference |
| `inputs/src/lib/legacyPricing.ts` | the dead class |
| `inputs/src/routes/CartView.svelte` | a route component — the entry point |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `lib.Cart` | referenced by the `CartView` component's `cart: Cart` declaration |
| `routes.CartView` | **no** incoming references — the router renders it, not another component. Named in `entry_points:` |
| `lib.LegacyPriceTable` | no incoming references and no hatch. **The violation.** |
...
| --- | --- | --- |
| `lib.Cart` | `{routes.CartView}` | referenced → silent |
| `routes.CartView` | `{}` (empty) | exempt: `entry_points:` |
| `lib.LegacyPriceTable` | `{}` (empty) | **dangling → fires** |
...
| --- | --- |
| `lib.Cart` | The route component declares `let cart: Cart`. |
| `routes.CartView` | Listed in `entry_points:`. **Every route component in a SvelteKit app has this shape** — nothing in the project renders it, so without the hatch the whole `routes/` tree would be reported. `"routes.**"` is the practical entry. |
...
```bash
make test-case CASE=check/dangling-classes/svelte
make debug-case CASE=check/dangling-classes/svelte
```
