# `dangling-classes` — dead code versus entry points (C#)
...
| `inputs/src/Orders/Cart.cs` | a class with an incoming reference |
| `inputs/src/Orders/Checkout.cs` | an entry point |
| `inputs/src/Orders/LegacyPricing.cs` | the dead class |
| `inputs/src/Web/Controller.cs` | a framework-derived class |
...
| `Web.OrderController` | derives from the configured `BaseCommand` framework base |
...
| `Web.OrderController` | `BaseCommand` is not a project class — the rule matches the base by **short name**, so an unresolved external base still exempts the subclass. |
...
```bash
make test-case CASE=check/dangling-classes/csharp
make debug-case CASE=check/dangling-classes/csharp
```
