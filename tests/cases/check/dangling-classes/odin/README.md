# `dangling-classes` — dead code versus entry points (Odin)
...
| `inputs/src/orders/cart.odin` | a struct with an incoming reference |
| `inputs/src/orders/checkout.odin` | an entry point |
| `inputs/src/orders/legacy.odin` | the dead struct |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `orders.Cart` | referenced by `CheckoutService`'s `cart` field |
...
Every file here declares `package orders`, because in Odin the package **is** the directory — three files, one package. The reference the rule sees is the struct field `cart: Cart`; `string` and `int` are not project types and never resolve.
...
```bash
make test-case CASE=check/dangling-classes/odin
make debug-case CASE=check/dangling-classes/odin
```
