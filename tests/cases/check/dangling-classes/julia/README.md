# `dangling-classes` — dead code versus entry points (Julia)
...
| `inputs/src/orders/Orders.jl` | one module holding all three structs |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `orders.Orders.Cart` | referenced by `CheckoutService`'s `cart::Cart` field |
| `orders.Orders.CheckoutService` | **no** incoming references, but named in `entry_points:` |
| `orders.Orders.LegacyPriceTable` | no incoming references and no hatch. **The violation.** |
...
[
  {
    "element": "orders.Orders.LegacyPriceTable",
...
| --- | --- | --- |
| `orders.Orders.Cart` | `{orders.Orders.CheckoutService}` | referenced → silent |
| `orders.Orders.CheckoutService` | `{}` (empty) | exempt: `entry_points:` |
| `orders.Orders.LegacyPriceTable` | `{}` (empty) | **dangling → fires** |
...
| --- | --- |
| `orders.Orders.Cart` | Referenced by `CheckoutService`. |
| `orders.Orders.CheckoutService` | Listed in `entry_points:` — note the **doubly-nested** qualified name that `entry_points:` must spell exactly. |

### Julia-specific notes

All three structs live in one `module Orders` inside `orders/`, so the qualified names nest twice: `orders.Orders.Cart`. Getting that wrong is the commonest Julia configuration mistake — an `entry_points:` entry of `orders.CheckoutService` would match nothing and the entry point would be reported as dead code.
...
```bash
make test-case CASE=check/dangling-classes/julia
make debug-case CASE=check/dangling-classes/julia
```
