# `dangling-classes` — dead code versus entry points (Python)
...
| `inputs/src/orders/cart.py` | a class with an incoming reference |
| `inputs/src/orders/checkout.py` | an entry point |
| `inputs/src/orders/legacy_pricing.py` | the dead class |
| `inputs/src/web/controller.py` | a framework-derived class |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `orders.Cart` | referenced by `CheckoutService.__init__` — has an incoming edge |
| `orders.CheckoutService` | **no** incoming references, but named in `entry_points:` — the DI container wires it |
| `orders.LegacyPriceTable` | no incoming references and no hatch. **The violation.** |
| `web.OrderController` | no incoming references either, but derives from the configured `BaseCommand` framework base |
...
| --- | --- |
| `orders.Cart` | It has an incoming reference from `CheckoutService`. |
| `orders.CheckoutService` | Listed in `entry_points:`. Without that line it would fire — it references `Cart` but nothing references it, which is precisely the shape clause 2 describes. |
| `web.OrderController` | `BaseCommand` is in `framework_bases:`. Note that setting the option **replaces** the built-in list, so the Unity defaults are no longer in effect here. |
...
```bash
make test-case CASE=check/dangling-classes/python
make debug-case CASE=check/dangling-classes/python
```
