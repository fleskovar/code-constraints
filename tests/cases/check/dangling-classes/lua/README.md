# `dangling-classes` — dead code versus entry points (Lua)
...
| `inputs/src/orders/checkout.lua` | an entry point |
| `inputs/src/orders/legacy.lua` | the dead table |
| `inputs/src/orders/service.lua` | a table with an incoming reference |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `orders.Service` | used as `CheckoutService`'s metatable base — an incoming edge |
...
| --- | --- | --- |
| `orders.Service` | `{orders.CheckoutService}` | referenced → silent |
...
| --- | --- |
| `orders.Service` | It is `CheckoutService`'s base. |
| `orders.CheckoutService` | Listed in `entry_points:`. |

### Lua-specific notes

⚠️ **Read this before turning the rule on for a Lua project.** Lua fields carry no type annotation and its function parameters are untyped, so clause 1's attribute, parameter and return paths all resolve to nothing. In practice **a metatable base is the only incoming edge a Lua model has**, which means `dangling-classes` will flag almost every class in a real Lua codebase. That is why this case builds its "referenced" row out of inheritance. Treat the rule as a `warning` in Lua and lean hard on `entry_points:`.
...
```bash
make test-case CASE=check/dangling-classes/lua
make debug-case CASE=check/dangling-classes/lua
```
