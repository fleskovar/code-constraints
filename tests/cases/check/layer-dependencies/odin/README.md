# `layer-dependencies` — the domain depends on nothing (Odin)
...
| `inputs/src/app/handler.odin` | the application layer |
| `inputs/src/domain/order.odin` | the violating struct |
| `inputs/src/infrastructure/sql.odin` | the forbidden target |
| `inputs/src/support/logger.odin` | a deliberately untagged struct |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `domain.Order` | `//@cdec layer("domain")`, has an infrastructure field. **The violation.** |
| `infrastructure.SqlConnection` | `//@cdec layer("infrastructure")` — the forbidden target |
...
Odin has no construct to hang a no-op decorator on — the compiler *rejects* unknown `@(...)` attributes — so the tag rides in a namespaced comment placed where a decorator would go. The `@cdec` prefix plays the gating role a shim import plays in Python or C#. ⚠️ **Line adjacency is required**: a blank line between the comment and the declaration detaches the tag, and the rule silently stops firing.
...
```bash
make test-case CASE=check/layer-dependencies/odin
make debug-case CASE=check/layer-dependencies/odin
```
