# `forbidden-package-references` — catalog is a leaf package (TypeScript)
...
| `inputs/src/catalog/author.ts` | an intra-package reference |
| `inputs/src/catalog/book.ts` | the violating class — an `Order[]` property |
| `inputs/src/orders/order.ts` | the allowed direction |
| `inputs/src/users/customer.ts` | a forbidden target nothing points at |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `catalog.Book` | `orders: Order[]` → `orders.Order`. **The violation.** |
| `catalog.Author` | `books: Book[]` → `catalog.Book`: a self-edge, dropped |
| `orders.Order` | `item: Book`: the permitted direction |
| `users.Customer` | in `to:`, but nothing in `catalog` references it |
...
| --- | --- |
| `users.Customer` | Nothing in `catalog` references it. |
...
```bash
make test-case CASE=check/forbidden-package-references/typescript
make debug-case CASE=check/forbidden-package-references/typescript
```
