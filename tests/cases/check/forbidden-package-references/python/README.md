# `forbidden-package-references` — catalog is a leaf package (Python)
...
| `inputs/src/catalog/author.py` | an intra-package reference |
| `inputs/src/catalog/book.py` | the violating class — a `list[Order]` attribute |
| `inputs/src/orders/order.py` | the allowed direction |
| `inputs/src/users/customer.py` | a forbidden target nothing points at |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `catalog.Book` | `self.orders: list[Order]` → `orders.Order`. **The violation.** |
| `catalog.Author` | `list[Book]` → `catalog.Book`: a self-edge, dropped by clause 3 |
| `orders.Order` | `Book` attribute → `catalog.Book`: the permitted direction |
| `users.Customer` | in `to:`, but no `catalog` class references it |
...
| --- | --- |
| `users.Customer` | Nothing in `catalog` references it, so the `catalog -> users` edge never exists. |
| `catalog.Author` | Its only edge is intra-package. |

### Python-specific notes

The Python parser also records the imported name as a body-level dependency (`Book.dependencies == ['Order']`), so this edge has two independent sources — the `list[Order]` attribute *and* the import. That is why removing the collection unwrapping alone would not make this case go green: the edge survives via the dependency. Both paths are part of clause 1.
...
```bash
make test-case CASE=check/forbidden-package-references/python
make debug-case CASE=check/forbidden-package-references/python
```
