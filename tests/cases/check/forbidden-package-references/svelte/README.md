# `forbidden-package-references` — catalog is a leaf package (Svelte 5)
...
| `inputs/src/catalog/AuthorCard.svelte` | an intra-package child |
| `inputs/src/catalog/BookCard.svelte` | the violating component — renders a child from another package |
| `inputs/src/orders/OrderRow.svelte` | the forbidden target component |
| `inputs/src/users/CustomerCard.svelte` | a forbidden target nothing renders |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `catalog.BookCard` | renders `<OrderRow />` → `orders.OrderRow`. **The violation.** |
| `catalog.AuthorCard` | renders `<BookCard />`: a self-edge, dropped by clause 3 |
| `orders.OrderRow` | the target; renders no child components |
| `users.CustomerCard` | in `to:`, but nothing in `catalog` renders it |
...
| --- | --- | --- |
| `catalog.BookCard -> orders.OrderRow` | `catalog -> orders` | ✅ |
| `catalog.AuthorCard -> catalog.BookCard` | `catalog -> catalog` | ❌ clause 3 |
| `orders.OrderRow -> —` | `—` | no child components |
...
| --- | --- |
| `users.CustomerCard` | Nothing renders it. |
| `catalog.AuthorCard` | Its only edge is intra-package. |

### Svelte 5-specific notes

A Svelte **component is a class** in the model, and rendering a child is the edge: the parser records the imported child as an attribute typed with the child's qualified name (`_orderRow: orders.OrderRow`). So "which package may render which" is expressible as an ordinary dependency rule, with no Svelte-specific machinery.
...
```bash
make test-case CASE=check/forbidden-package-references/svelte
make debug-case CASE=check/forbidden-package-references/svelte
```
