# `forbidden-package-references` — catalog is a leaf package (Lua)
...
| `inputs/src/catalog/book.lua` | the violating table — an `__index` metatable base |
| `inputs/src/catalog/paperback.lua` | an intra-package base |
| `inputs/src/orders/order.lua` | the forbidden target |
| `inputs/src/users/customer.lua` | a forbidden target nothing points at |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `catalog.Book` | `setmetatable({}, { __index = Order })` → base `Order`. **The violation.** |
| `catalog.Paperback` | base `Book`: a self-edge, dropped by clause 3 |
| `orders.Order` | the target; references nothing itself |
| `users.Customer` | in `to:`, but nothing in `catalog` derives from it |
...
| `catalog.Paperback -> catalog.Book` | `catalog -> catalog` | ❌ clause 3 |
| `orders.Order -> —` | `—` | no outgoing edges |
...
| --- | --- |
| `users.Customer` | Nothing derives from it. |
| `catalog.Paperback` | Its only edge is intra-package. |

### Lua-specific notes

**Lua fields carry no type annotation**, so an attribute never resolves to a class and clause 1's attribute path is dead here. A metatable base is the one reference a Lua model can see, which is why this case is built out of `setmetatable({}, { __index = T })` rather than out of fields. That is a real property of the language, not a shortcut: in Lua, `forbidden-package-references` effectively polices the inheritance graph.
...
```bash
make test-case CASE=check/forbidden-package-references/lua
make debug-case CASE=check/forbidden-package-references/lua
```
