# `forbidden-package-references` — catalog is a leaf package (C#)
...
| `inputs/src/Catalog/Author.cs` | an intra-package reference |
| `inputs/src/Catalog/Book.cs` | the violating class — a `List<Order>` property |
| `inputs/src/Orders/Order.cs` | the allowed direction |
| `inputs/src/Users/Customer.cs` | a forbidden target nothing points at |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| `Catalog.Book` | `List<Order>` property → `Orders.Order`. **The violation.** |
| `Catalog.Author` | `List<Book>` → `Catalog.Book`: a self-edge, dropped by clause 3 |
| `Orders.Order` | `Book` property → `Catalog.Book`: the permitted direction |
...
```bash
make test-case CASE=check/forbidden-package-references/csharp
make debug-case CASE=check/forbidden-package-references/csharp
```
