# `@locked` — a semantic change trips the lock (C#)
...
| `inputs/src/Orders/Billing.cs` | the same implementation with one semantic edit (a changed string literal) |
...
```bash
make test-case CASE=lock/locked/csharp/changed
make debug-case CASE=lock/locked/csharp/changed
```
