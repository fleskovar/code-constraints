# `@locked` — deleting the tag does not escape the lock (C#)
...
| `inputs/src/Orders/Billing.cs` | the identical body with the tag deleted |
| `inputs/baseline/Orders/Billing.cs` | the approved implementation, tagged |
...
hand; it is `cdec check --automatic-exceptions locks --force`, which prunes the entry and leaves
...
```bash
make test-case CASE=lock/locked/csharp/unlocked
make debug-case CASE=lock/locked/csharp/unlocked
```
