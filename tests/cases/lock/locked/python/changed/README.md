# `@locked` — a semantic change trips the lock (Python)
...
| `inputs/src/orders/billing.py` | the same implementation with one semantic edit (a changed string literal) |
...
| --- | --- |
| ``orders.Receipt.formatted`` | the locked method. Exactly one edit: `" line(s) - total "` → `" item(s) - total "` |
...
**Step 1 — the edit.** `" line(s) - total "` → `" item(s) - total "` — a changed string literal. Nothing else in the file differs, which is
...
**Step 3 — the finding (clause 6).** Kind `changed`, target `orders.Receipt.formatted`. The real report
...
```bash
make test-case CASE=lock/locked/python/changed
make debug-case CASE=lock/locked/python/changed
```
