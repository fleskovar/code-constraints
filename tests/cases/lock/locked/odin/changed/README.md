# `@locked` — a semantic change trips the lock (Odin)
...
| `inputs/src/billing/invoice.odin` | the same implementation with one semantic edit (an added expression) |
...
| --- | --- |
| ``billing.Invoice.formatted`` | the locked proc. Exactly one edit: `return inv.id` → `return inv.id + "!"` |
...
```bash
make test-case CASE=lock/locked/odin/changed
make debug-case CASE=lock/locked/odin/changed
```
