# `@locked` — reformatting does not trip a lock (Julia)

## What this proves

A lock is an **AST identity, not a line range**. Moving the element, reformatting it, rewriting comments and docstrings, and inserting unrelated code above it all leave the digest untouched. An empty baseline is the assertion here — and it is the property the whole feature rests on, because without it the rule would fire on every reformat and teams would switch it off.
...
| `inputs/src/billing/Invoice.jl` | the same implementation, cosmetically rewritten |
| `inputs/baseline/billing/Invoice.jl` | the approved implementation |

### Why each element is there

| Element | Demonstrates |
| --- | --- |
| ``const UNRELATED = 1`` | **inserted above** the locked function. Clause 2 |
| `a new `# comment` in the body` | clause 3 drops comments |
| ``billing.Billing.Invoice.formatted`` | the locked function. Its body is **semantically identical** on both sides |
...
Digesting `inputs/baseline/` records one entry: target `billing.Billing.Invoice.formatted`, algo `jl-ts/1`.

**Step 1 — classify every edit in `inputs/src/` (clauses 2–4).**

| Edit | Why it is cosmetic | Effect on the digest |
| --- | --- | --- |
| `const UNRELATED = 1` | **inserted above** the locked function. Clause 2 | invisible |
...
**Julia's tag wraps the definition** instead of preceding it, and the flattened-macro grammar makes it impossible to re-serialise the chain minus one tag — so the digest is taken over the *unwrapped* definition, with non-rule macros (`@inline`) folded back in via `foreign_macros` so they stay significant.
...
- **It would catch:** a fingerprinter that started including position attributes, one that stopped dropping comments, or one that failed to strip the `@locked` tag — any of which would make the ledger unable to record a stable baseline in the first place.
...
```bash
make test-case CASE=lock/locked/julia/reformat
make debug-case CASE=lock/locked/julia/reformat
```

**Start here:** breakpoint in `tests/case_runner.py::_run_lock`, then step into the engine. Compare `collect_targets(baseline, "julia")` with `collect_targets(src, "julia")` and diff the two digests.

## When to change this case

A red run is a regression until proven otherwise — do not regenerate the baseline to get green. If the requirement genuinely changed, add a new case for the new behaviour and retire this one explicitly. Regenerating (`UPDATE_BASELINES=1 make test-case CASE=lock/locked/julia/reformat`) produces a diff a human reads line by line, in a commit that changes baselines and nothing else.
