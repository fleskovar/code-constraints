# `@locked` — a semantic change trips the lock (Julia)
...
| `inputs/src/billing/Invoice.jl` | the same implementation with one semantic edit (an added expression) |
...
| --- | --- |
| ``billing.Billing.Invoice.formatted`` | the locked function. Exactly one edit: `return inv.id` → `return inv.id * "!"` |
...
**Step 1 — the edit.** `return inv.id` → `return inv.id * "!"` — an added expression. Nothing else in the file differs, which is
...
**Step 3 — the finding (clause 6).** Kind `changed`, target `billing.Billing.Invoice.formatted`. The real report
...
```bash
make test-case CASE=lock/locked/julia/changed
make debug-case CASE=lock/locked/julia/changed
```
