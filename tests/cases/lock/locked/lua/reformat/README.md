# `@locked` — reformatting does not trip a lock (Lua)
...
| `inputs/src/billing/invoice.lua` | the same implementation, cosmetically rewritten |
...
| --- | --- |
| ``local UNRELATED = 1`` | **inserted above** the locked function. Clause 2 |
| `a new `-- comment` in the body` | clause 3 drops comments |
| ``billing.Invoice.formatted`` | the locked method. Its body is **semantically identical** on both sides |
...
Digesting `inputs/baseline/` records one entry: target `billing.Invoice.formatted`, algo `lua-ts/1`.

**Step 1 — classify every edit in `inputs/src/` (clauses 2–4).**

| Edit | Why it is cosmetic | Effect on the digest |
| --- | --- | --- |
| `local UNRELATED = 1` | **inserted above** the locked function. Clause 2 | invisible |
...
```bash
make test-case CASE=lock/locked/lua/reformat
make debug-case CASE=lock/locked/lua/reformat
```

**Start here:** breakpoint in `tests/case_runner.py::_run_lock`, then step into the engine. Compare `collect_targets(baseline, "lua")` with `collect_targets(src, "lua")` and diff the two digests.

## When to change this case

A red run is a regression until proven otherwise — do not regenerate the baseline to get green. If the requirement genuinely changed, add a new case for the new behaviour and retire this one explicitly. Regenerating (`UPDATE_BASELINES=1 make test-case CASE=lock/locked/lua/reformat`) produces a diff a human reads line by line, in a commit that changes baselines and nothing else.
