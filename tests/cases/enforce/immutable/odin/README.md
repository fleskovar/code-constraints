# `@immutable` — no field reassignment after construction (Odin)
...
**Odin has no constructors**, so there is no constructor exemption to apply: a struct is built with a composite literal at the call site, not by a method. Every proc that assigns to a receiver field is therefore a violation. Note the contrast with `with_discount`, which *returns* `Receipt{...}` — a composite literal is a construction, not an assignment to the receiver, so it is silent.
...
```bash
make test-case CASE=enforce/immutable/odin
make debug-case CASE=enforce/immutable/odin
```
