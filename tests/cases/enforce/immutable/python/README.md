# `@immutable` — no field reassignment after construction (Python)
...
| `the constructor` | Exempt by clause 3 (any method whose name is not `__init__`). |
...
```bash
make test-case CASE=enforce/immutable/python
make debug-case CASE=enforce/immutable/python
```
