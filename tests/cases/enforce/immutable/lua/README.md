# `@immutable` — no field reassignment after construction (Lua)
...
| `orders.Receipt.apply_discount` | reassigns `total` outside the constructor. **The violation.** |
...
3. The constructor is exempt: assignment there is *initialisation*. Here that means a static function named `new`, `create`, `init`, `_init` or `_new`.
...
5. Lua has no `__init__` to privilege, so the constructor convention is **named explicitly** in the analyzer (`_CONSTRUCTOR_NAMES`) rather than guessed per project.
...
| `the constructor` | Exempt by clause 3 (a static function named `new`, `create`, `init`, `_init` or `_new`). |

### Lua-specific notes

`Receipt.new` assigns `self.total` and is silent purely because it is called `new`. Rename it to `build` and this case grows a second finding — which is worth knowing before you adopt the tag on a codebase with its own constructor naming.
...
```bash
make test-case CASE=enforce/immutable/lua
make debug-case CASE=enforce/immutable/lua
```
