# `@no_instantiation` — an orchestrator wires, it does not build (Odin)
...
| `inputs/src/orders/checkout.odin` | source under test |
...
```bash
make test-case CASE=enforce/no-instantiation/odin
make debug-case CASE=enforce/no-instantiation/odin
```
