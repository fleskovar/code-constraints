# `@no_instantiation` — an orchestrator wires, it does not build (Lua)
...
| `inputs/src/orders/models.lua` | source under test |
...
| `orders.CheckoutService.checkout` | builds `AuditEntry.new("checkout")` — on the allow-list, so silent |
| `orders.CheckoutService.quick_receipt` | builds `Receipt.new(total)` — not on the list. **The violation.** |
...
| --- | --- | --- | --- |
| `checkout` | `AuditEntry.new("checkout")` → `AuditEntry` | ✅ | silent |
| `quick_receipt` | `Receipt.new(total)` → `Receipt` | ❌ | **Fires** |
...
```bash
make test-case CASE=enforce/no-instantiation/lua
make debug-case CASE=enforce/no-instantiation/lua
```
