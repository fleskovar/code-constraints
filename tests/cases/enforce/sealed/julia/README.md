# `@sealed` — a value object may not be extended (Julia)
...
| --- | --- |
| `orders.Orders.Receipt` | carries the tag. A value object whose invariants cannot survive being extended |
| `orders.Orders.Discounted` | `struct Discounted <: Receipt`. **The violation.** |
| `orders.Orders.Annotated` | **composition** — holds a `Receipt` in a field rather than extending it. The sanctioned alternative, and silent |
...
[
  {
    "detail": "base:Receipt",
    "element": "orders.Orders.Discounted",
    "rule": "sealed"
  }
]
```
...
| --- | --- | --- |
| `orders.Orders.Receipt` | `[]` | — (and clause 4 would exclude it anyway) |
| `orders.Orders.Annotated` | `[]` | ❌ — it holds a `Receipt`, it does not extend one |
| `orders.Orders.Discounted` | `['Receipt']` | ✅ **Fires** |

**Step 3 — the finding (clause 5).** Attributed to `orders.Orders.Discounted`, discriminated
...
| --- | --- |
| `orders.Orders.Annotated` | Composition is not inheritance. Its `Receipt` field creates a *reference* edge, which `cdec check` can police, but never a base. |
| `orders.Orders.Receipt` | A class does not subclass itself (clause 4). |

### Julia-specific notes

Julia can only subtype an **abstract** type, so that is where `@sealed` earns its keep: it declares the hierarchy closed. Tagging a concrete `struct` would be harmless but pointless, since the compiler already forbids extending it.
...
```bash
make test-case CASE=enforce/sealed/julia
make debug-case CASE=enforce/sealed/julia
```
