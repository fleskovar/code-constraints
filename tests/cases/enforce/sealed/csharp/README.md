# `@sealed` — a value object may not be extended (C#)
...
| `inputs/src/Orders/AnnotatedReceipt.cs` | source under test |
| `inputs/src/Orders/DiscountedReceipt.cs` | source under test |
...
```bash
make test-case CASE=enforce/sealed/csharp
make debug-case CASE=enforce/sealed/csharp
```
