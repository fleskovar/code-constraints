# `@sealed` — a value object may not be extended (Python)
...
| `inputs/src/orders/annotated.py` | source under test |
| `inputs/src/orders/discounted.py` | source under test |
| `inputs/src/orders/receipt.py` | source under test |
...
| `orders.DiscountedReceipt` | `class DiscountedReceipt(Receipt)`. **The violation.** |
| `orders.AnnotatedReceipt` | **composition** — holds a `Receipt` in a field rather than extending it. The sanctioned alternative, and silent |
...
[
  {
    "detail": "base:Receipt",
    "element": "orders.DiscountedReceipt",
    "rule": "sealed"
  }
]
```
...
| `orders.AnnotatedReceipt` | `[]` | ❌ — it holds a `Receipt`, it does not extend one |
| `orders.DiscountedReceipt` | `['Receipt']` | ✅ **Fires** |

**Step 3 — the finding (clause 5).** Attributed to `orders.DiscountedReceipt`, discriminated
...
| --- | --- |
| `orders.AnnotatedReceipt` | Composition is not inheritance. Its `Receipt` field creates a *reference* edge, which `cdec check` can police, but never a base. |
...
```bash
make test-case CASE=enforce/sealed/python
make debug-case CASE=enforce/sealed/python
```
