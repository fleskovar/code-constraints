# `@sealed` — a value object may not be extended (Odin)
...
| `orders.Discounted` | `using base: Receipt`. **The violation.** |
...
Odin has no inheritance. `using base: T` is its **subtype embedding**: it splices the base's fields in and makes the struct usable where the base is expected. The parser maps it to `bases`, so Engine B sees through it to exactly the finding a Python `class D(R)` produces. `Annotated` holds a `Receipt` as an ordinary field and is silent — the difference between `using base:` and a plain field is the whole distinction the tag cares about.
...
```bash
make test-case CASE=enforce/sealed/odin
make debug-case CASE=enforce/sealed/odin
```
