# `@factory` — one way to build a receipt (Python)
...
| `inputs/src/orders/factory.py` | source under test |
...
```bash
make test-case CASE=enforce/factory/python
make debug-case CASE=enforce/factory/python
```
