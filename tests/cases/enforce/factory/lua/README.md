# `@factory` — one way to build a receipt (Lua)
...
| `inputs/src/orders/checkout.lua` | source under test |
| `inputs/src/orders/factory.lua` | source under test |
...
| --- | --- |
| `orders.ReceiptFactory` | carries `creates=["Receipt"]`. Its own `Receipt.new(total)` is permitted by clause 4 — this *is* the factory |
| `orders.CheckoutService.checkout` | delegates to the factory. A method call, not a construction — silent |
| `orders.CheckoutService.quick_receipt` | a shortcut: `Receipt.new(total)` outside the designated factory. **The violation.** |
...
2. Construction detection in this language is **idiom-based**. Lua has no construction syntax at all. Two idioms are recognised: `T.new(...)` / `T:new(...)` where `T` names a project class, and a direct `setmetatable(tbl, T)`. **A bare `T(...)` is NOT construction** — in Lua that is an ordinary `__call`. The analyzer therefore under-reports on unusual constructor conventions.
...
| --- | --- | --- | --- |
| `orders.ReceiptFactory.for_total` | `Receipt.new(total)` | ✅ | permitted (clause 4) |
| `orders.CheckoutService.checkout` | *(no construction — it calls the factory)* | — | silent |
| `orders.CheckoutService.quick_receipt` | `Receipt.new(total)` | ❌ | **Fires** |
...
```bash
make test-case CASE=enforce/factory/lua
make debug-case CASE=enforce/factory/lua
```
