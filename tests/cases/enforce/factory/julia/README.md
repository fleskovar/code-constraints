# `@factory` — one way to build a receipt (Julia)
...
| --- | --- |
| `orders.Orders.ReceiptFactory` | carries `creates=["Receipt"]`. Its own `Receipt(total)` is permitted by clause 4 — this *is* the factory |
| `orders.Orders.CheckoutService.checkout` | delegates to the factory. A method call, not a construction — silent |
| `orders.Orders.CheckoutService.quick_receipt` | a shortcut: `Receipt(total)` outside the designated factory. **The violation.** |
...
`orders.Orders.ReceiptFactory`.

**Step 2 — find every construction of `Receipt` (clause 2).**

| Site | Expression | Inside the designated factory? | Verdict |
| --- | --- | --- | --- |
| `orders.Orders.ReceiptFactory.for_total` | `Receipt(total)` | ✅ | permitted (clause 4) |
| `orders.Orders.CheckoutService.checkout` | *(no construction — it calls the factory)* | — | silent |
| `orders.Orders.CheckoutService.quick_receipt` | `Receipt(total)` | ❌ | **Fires** |
...
| --- | --- |
| `orders.Orders.ReceiptFactory.for_total` | This is the designated factory; clause 4 permits construction anywhere inside it. |
| `orders.Orders.CheckoutService.checkout` | Calling a method that returns a `Receipt` is not constructing one. |

### Julia-specific notes

Note the language's detection limit, restated: a bare `Receipt(...)` in Lua would **not** be caught, because that is `__call`, not construction. In Julia the opposite risk applies — a call is only a construction when its callee is a type *this parse saw*, so a `Receipt` defined outside the scanned tree would be missed.
...
```bash
make test-case CASE=enforce/factory/julia
make debug-case CASE=enforce/factory/julia
```
